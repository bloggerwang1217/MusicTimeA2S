#!/bin/bash
# =============================================================================
# Slurm Parallel ASAP MV2H Evaluation — two-phase pipeline
# =============================================================================
#
# Phase 1 (render): slurm array, parallel per-chunk score→MIDI render. The
#                   root of ground-truth score MIDI is read-only and shared by every shard.
...
#   PRED_DIR=... GROUNDING=... GT_SCORE_MIDI_ROOT=... OUTPUT_DIR=... \
#     bash src/evaluation/slurm_eval_window.sh
#
#   # Override per-job size (default 100, matching slurm_eval_syn.sh)
#   CHUNKS_PER_JOB=200 PRED_DIR=... bash src/evaluation/slurm_eval_window.sh
#
...
PRED_DIR="${PRED_DIR:?set the directory of Ours kern excerpts}"
prediction_args=(--pred-dir "$PRED_DIR")
prediction_input="$PRED_DIR"
GT_SCORE_MIDI_ROOT="${GT_SCORE_MIDI_ROOT:-data/experiments/asap102/native_reference}"
GROUNDING="${GROUNDING:?set the prepared stride-five grounding manifest}"
...
if [ "$1" = "--render-worker" ]; then
    ...
    poetry run python -m src.evaluation.eval_window \
        --phase render \
        "${prediction_args[@]}" \
        --grounding "$GROUNDING" \
        --gt-score-midi-root "$GT_SCORE_MIDI_ROOT" \
...
if [ "$1" = "--eval-worker" ]; then
    ...
    if [ "$DONE_COUNT" -eq "$N_JOBS" ]; then
        ...
        if mkdir "$MERGE_LOCK" 2>/dev/null; then
            ...
            if poetry run python -m src.evaluation.eval_window \
                --phase merge \
                "${prediction_args[@]}" \
                --gt-score-midi-root "$GT_SCORE_MIDI_ROOT" \
                --chunk-timeout "$CHUNK_TIMEOUT" \
...
for name in GROUNDING GT_SCORE_MIDI_ROOT OUTPUT_DIR; do
    if [ -z "${!name}" ]; then
...
REQUIRED_PATHS=("$prediction_input" "$MV2H_BIN" "$GROUNDING" "$GT_SCORE_MIDI_ROOT")
...
echo "Prediction input: $prediction_input"
echo "Grounding:       $GROUNDING ($TOTAL_CHUNKS chunks)"
echo "GT score MIDI:  $GT_SCORE_MIDI_ROOT"
echo "Output dir:      $OUTPUT_DIR"
echo "Chunks per job:  $CHUNKS_PER_JOB"
echo "Array size:      $N_JOBS (0-$MAX_IDX)"
echo ""

LOG_DIR="logs/eval_window_$(basename "$OUTPUT_DIR")"
mkdir -p "$LOG_DIR"

export PRED_DIR GROUNDING GT_SCORE_MIDI_ROOT \
