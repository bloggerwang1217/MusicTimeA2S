"""
Vendored from sony/hFT-Transformer model/model_spec2midi.py (release ismir2023).
Kept byte-identical to upstream because the published weights load by key name;
re-vendor to update, don't hand-edit.
"""
...
class Model_SPEC2MIDI(nn.Module):
    ...
    def tap_only(self, input_spec):
        """Only compute midi_time_tap, skip all output heads."""
        enc_vector = self.encoder_spec2midi(input_spec)
        return self.decoder_spec2midi.tap_only(enc_vector)
    ...
    def forward(self, input_spec):
        ...
        output_onset_A, output_offset_A, output_mpe_A, output_velocity_A, attention, output_onset_B, output_offset_B, output_mpe_B, output_velocity_B, midi_time_tap = self.decoder_spec2midi(enc_vector)
        #midi_time_tap = [batch_size, n_frame, n_note, hid_dim] (8, 128, 88, 256)
        ...
        return output_onset_A, output_offset_A, output_mpe_A, output_velocity_A, attention, output_onset_B, output_offset_B, output_mpe_B, output_velocity_B, midi_time_tap
...
class Encoder_SPEC2MIDI(nn.Module):
    def __init__(self, n_margin, n_frame, n_bin, cnn_channel, cnn_kernel, hid_dim, n_layers, n_heads, pf_dim, dropout, device):
        ...
        self.register_buffer('_pos_freq_base', torch.arange(n_bin, device=device))
    ...
    def forward(self, spec_in):
        ...
        # position coding
        pos_freq = self._pos_freq_base.unsqueeze(0).expand(batch_size * self.n_frame, -1)
...
class Decoder_SPEC2MIDI(nn.Module):
    def __init__(self, n_frame, n_bin, n_note, n_velocity, hid_dim, n_layers, n_heads, pf_dim, dropout, device):
        ...
        self.register_buffer('_pos_note_base', torch.arange(n_note, device=device))
        self.register_buffer('_pos_frame_base', torch.arange(n_frame, device=device))

    def tap_only(self, enc_spec):
        """Only compute midi_time_tap, skip all output heads."""
        batch_size = enc_spec.shape[0]
        enc_spec = enc_spec.reshape([batch_size*self.n_frame, self.n_bin, self.hid_dim])

        pos_freq = self._pos_note_base.unsqueeze(0).expand(batch_size * self.n_frame, -1)
        midi_freq = self.pos_embedding_freq(pos_freq)

        midi_freq, _ = self.layer_zero_freq(enc_spec, midi_freq)
        for layer_freq in self.layers_freq:
            midi_freq, _ = layer_freq(enc_spec, midi_freq)
        ...
        pos_time = self._pos_frame_base.unsqueeze(0).expand(batch_size * self.n_note, -1)
        ...
        return midi_time.reshape(batch_size, self.n_note, self.n_frame, self.hid_dim).permute(0, 2, 1, 3).contiguous()
    ...
    def forward(self, enc_spec):
        ...
        # midi_time after all time layers: [B*n_note, n_frame, hid_dim]
        midi_time_tap = midi_time.reshape(batch_size, self.n_note, self.n_frame, self.hid_dim).permute(0, 2, 1, 3).contiguous()

        return output_onset_freq, output_offset_freq, output_mpe_freq, output_velocity_freq, None, output_onset_time, output_offset_time, output_mpe_time, output_velocity_time, midi_time_tap
